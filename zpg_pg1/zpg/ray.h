...
struct Ray : RTCRay
{
	...
	bool hasCollided() {
		return this->geomID != RTC_INVALID_GEOMETRY_ID && this->tnear > 0.0f;
	}
	...
	Ray getReflected(Vector3 &normal) {
		return Ray(this->getIntersectPoint(), this->getDir().Reflect(normal), 0.01f);
	}

	Ray getRefracted(Vector3 &normal, float n1, float n2) {
		return Ray(this->getIntersectPoint(), this->getDir().Refract(normal, n1, n2), 0.01f);
	}
};
